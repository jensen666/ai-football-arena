...
test("setupKickoff 把球放回中圈并交给指定开球方", () => {
  ...
  const kicker = engine.teams.away.players.find((player) => player.id === engine.ball.holderId);
  assert.ok(kicker, "应存在开球手");
  assert.equal(kicker.x, 50);
  assert.equal(kicker.y, 50);
});
...
test("开球前 1 秒球固定在中圈且球员不提前移动", () => {
  const engine = setup();
  assert.equal(engine.state, "kickoff");
  const kicker = engine.teams.home.players.find((player) => player.id === engine.ball.holderId);
  const startX = kicker.x;
  const startY = kicker.y;
  // 推进 0.5 秒（准备阶段）
  for (let i = 0; i < 15; i += 1) engine.advanceTick();
  assert.equal(engine.state, "kickoff");
  assert.equal(engine.ball.x, 50);
  assert.equal(engine.ball.y, 50);
  assert.equal(kicker.x, startX);
  assert.equal(kicker.y, startY);
});

test("开球 1 秒后由开球手传球给队友并进入比赛中", () => {
  const engine = setup();
  assert.equal(engine.state, "kickoff");
  const kickerId = engine.ball.holderId;
  // 推进 1 秒以上
  for (let i = 0; i < 35; i += 1) engine.advanceTick();
  assert.equal(engine.state, "in_play");
  // 球可能已经飞出也可能已被接住，重点是球权已从开球手转移。
  assert.notEqual(engine.ball.holderId, kickerId, "球权应交给接球队友");
});
